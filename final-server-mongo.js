...
async function generatePDF(offerData) {
    console.log("📄 Generiram PDF za:", offerData.id);
    ...
    try {
        ...
        console.log("✅ PDF generiran, velikost:", pdfBuffer.length, "bajtov");
        return pdfBuffer;
    } catch (error) {
...
app.post('/api/offers/create', async (req, res) => {
    try {
        ...
        await newOffer.save();
        console.log("💾 Ponudba shranjena v MongoDB");
...
// API endpoint za prenos PDF-ja
app.get('/api/offers/:id/pdf', async (req, res) => {
    try {
        const offerId = req.params.id;
        const offer = await Offer.findOne({ id: offerId });
        
        if (!offer || !offer.pdfData) {
            return res.status(404).json({ success: false, error: 'PDF ne obstaja' });
        }
        
        res.set('Content-Type', 'application/pdf');
        res.set('Content-Disposition', `inline; filename="ponudba_${offerId}.pdf"`);
        ...
        <head>
            ...
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .btn-delete { background: #f44336; padding: 6px 12px; font-size: 12px; border: none; border-radius: 5px; cursor: pointer; }
            </style>
        </head>
        <body>
            <div class="container">
                ...
                <div class="controls">
                    <input type="text" class="search-box" id="searchInput" placeholder="🔍 Išči...">
                    <button onclick="loadOffers()">🔄 Osveži</button>
                    <button onclick="window.location.href='/'">➕ Nova ponudba</button>
                </div>
                <div class="table-container">
                    <table>
                        <thead><tr><th>Številka</th><th>Datum</th><th>Podjetje</th><th>Stranka</th><th>Skupaj (€)</th><th>Akcije</th></tr></thead>
